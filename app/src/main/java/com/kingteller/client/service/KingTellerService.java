...
public class KingTellerService extends Service {
	...
	private AMapLocationListener staffListener = new AMapLocationListener() {
		...
		public void onLocationChanged(AMapLocation location) {
			...
			//	KingTellerApplication.getApplication().setCurAddress(address);
				...
				/*写获取定位信息成功---调试代码 */
			//	CrashHandler.saveStaffLogInTextFile("没有获取定位信息!:" + erro, null, null, null, null, 1);
		...
		public void run() {
			if (!KingTellerJudgeUtils.isNetAvaliable(KingTellerService.this)){
				if(MumNetwork_OFF == 0){
					MumNetwork_OFF++;
					/*写入网络不可用---调试代码 */
				//	CrashHandler.saveStaffLogInTextFile("当前网络不可用!", null, null, null, null, 1);
				}
				MumNetwork_ON = 0;
				Log.e(TAG, "当前网络不可用！");
			}else{	
				if(MumNetwork_ON == 0){
					MumNetwork_ON++;
					/*写入网络不可用---调试代码 */
			//		CrashHandler.saveStaffLogInTextFile("当前网络正常!", null, null, null, null, 1);
				}
				MumNetwork_OFF = 0;
				Log.e(TAG, "当前网络正常！");
			}
	...
	public void onCreate() {
		...
		/*写service服务状态---调试代码 */
	//	CrashHandler.saveStaffLogInTextFile("service服务已经启动!", null, null, null, null, 1);
	...
	public void onDestroy() {
		...
		/*写service服务状态---调试代码 */
//		CrashHandler.saveStaffLogInTextFile("service服务已经停止!", null, null, null, null, 1);
